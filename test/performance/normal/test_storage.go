...
func transfer(i int) {
	action1 := tx.NewAction(contractID, "transfer", `["admin","testID",1]`)
	action2 := tx.NewAction(contractID, "test", `[]`)
	acc, _ := account.NewKeyPair(loadBytes(rootKey), crypto.Ed25519)
	trx := tx.NewTx([]*tx.Action{action1, action2}, []string{}, 5000000, 100, time.Now().Add(time.Second*time.Duration(10000)).UnixNano(), 0)
...
func publish() string {
	acc, _ := account.NewKeyPair(loadBytes(rootKey), crypto.Ed25519)
	codePath := "normal_test.js"
	abiPath := codePath + ".abi"
	sdk.SetAccount("admin", acc)
	sdk.SetServer("3.0.11.139:30002")
